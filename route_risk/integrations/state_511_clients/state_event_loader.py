...
from route_risk.integrations.state_511_clients.arizona_511_client import (
    fetch_arizona_511_event_groups,
)

from route_risk.integrations.state_511_clients.utah_udot_client import (
...
StateEventGroupFetcher = Callable[
    ...,
    EventGroups,
]


STATE_EVENT_GROUP_FETCHERS: Dict[
    str,
    StateEventGroupFetcher,
] = {
    "NV": fetch_nevada_511_event_groups,
    "AZ": fetch_arizona_511_event_groups,
    "UT": fetch_utah_udot_event_groups,
}
...
def fetch_event_groups_for_state(
    ...
    group_fetcher = (
        STATE_EVENT_GROUP_FETCHERS.get(
            normalized_code
        )
    )
...
if __name__ == "__main__":
    print_section_title(
        "STATE 511 EVENT GROUP LOADER TEST"
    )
    ...
    print(
        f"Active scoring events: "
        f"{len(active_events)}"
    )

    print(
        f"Upcoming disclosure events: "
    ...
    print_section_title(
        "END STATE 511 EVENT GROUP LOADER TEST"
    )